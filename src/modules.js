export const modules = [
  {
    id: 'hoc-vien',
    name: 'Học viên',
    shortDescription: 'Quản lý hồ sơ học viên và theo dõi trạng thái học tập tại cơ sở.',
    status: 'in-progress',
    plannedFeatures: [
      'Xem danh sách học viên',
      'Tìm kiếm học viên',
      'Lọc theo trạng thái học',
      'Xem chi tiết học viên',
      'Ghi chú chăm sóc',
    ],
    plannedData: [
      'Thông tin học viên',
      'Thông tin phụ huynh',
      'Trạng thái học',
      'Lịch sử ghi chú chăm sóc',
    ],
  },
  {
    id: 'khach-hang-tu-van',
    name: 'Khách hàng / Tư vấn',
    shortDescription: 'Theo dõi khách hàng tiềm năng, phụ huynh và quá trình tư vấn.',
    status: 'planned',
    plannedFeatures: [
      'Lưu thông tin khách hàng/phụ huynh',
      'Theo dõi trạng thái tư vấn',
      'Ghi chú lịch sử chăm sóc',
      'Chuyển khách hàng tiềm năng thành học viên',
    ],
    plannedData: [
      'Thông tin khách hàng',
      'Thông tin phụ huynh',
      'Trạng thái tư vấn',
      'Lịch sử chăm sóc',
    ],
  },
  {
    id: 'giao-vien',
    name: 'Giáo viên',
    shortDescription: 'Quản lý danh sách giáo viên, liên hệ và trạng thái làm việc.',
    status: 'planned',
    plannedFeatures: [
      'Xem danh sách giáo viên',
      'Xem thông tin liên hệ',
      'Theo dõi trạng thái làm việc',
      'Gắn giáo viên với lịch dạy',
    ],
    plannedData: [
      'Hồ sơ giáo viên',
      'Thông tin liên hệ',
      'Trạng thái làm việc',
      'Lịch dạy liên quan',
    ],
  },
  {
    id: 'nhan-vien',
    name: 'Nhân viên',
    shortDescription: 'Chuẩn bị khung quản lý nhân sự vận hành và phân quyền sau này.',
    status: 'planned',
    plannedFeatures: [
      'Xem danh sách nhân viên',
      'Phân loại nhân viên',
      'Ghi nhận vai trò làm việc',
      'Chuẩn bị cho phân quyền sau này',
    ],
    plannedData: [
      'Hồ sơ nhân viên',
      'Nhóm nhân sự',
      'Vai trò làm việc',
      'Trạng thái tài khoản tương lai',
    ],
  },
  {
    id: 'thoi-khoa-bieu',
    name: 'Thời khóa biểu',
    shortDescription: 'Chuẩn bị màn hình xem lịch dạy, lịch làm và lịch trực tại cơ sở.',
    status: 'planned',
    plannedFeatures: [
      'Xem lịch dạy',
      'Xem lịch làm/lịch trực',
      'Lọc theo ngày/tuần',
      'Lọc theo giáo viên/nhân viên',
    ],
    plannedData: [
      'Lịch theo ngày',
      'Lịch theo tuần',
      'Giáo viên phụ trách',
      'Nhân viên trực',
    ],
  },
  {
    id: 'hoc-phi',
    name: 'Học phí',
    shortDescription: 'Theo dõi học phí, gói học, số buổi và tình trạng thanh toán.',
    status: 'in-progress',
    plannedFeatures: [
      'Theo dõi gói học/khóa học',
      'Theo dõi số buổi',
      'Theo dõi đã đóng/còn nợ',
      'Cảnh báo sắp đến hạn/quá hạn',
    ],
    plannedData: [
      'Gói học',
      'Số buổi',
      'Khoản đã đóng',
      'Khoản còn nợ',
      'Mốc đến hạn',
    ],
  },
  {
    id: 'thu-chi',
    name: 'Thu chi',
    shortDescription: 'Chuẩn bị khung ghi nhận các khoản thu, khoản chi và tổng hợp theo ngày.',
    status: 'planned',
    plannedFeatures: [
      'Ghi nhận khoản thu',
      'Ghi nhận khoản chi',
      'Tổng hợp thu chi theo ngày',
    ],
    plannedData: [
      'Khoản thu',
      'Khoản chi',
      'Ngày giao dịch',
      'Người ghi nhận',
    ],
  },
  {
    id: 'so-quy',
    name: 'Sổ quỹ',
    shortDescription: 'Theo dõi tổng thu, tổng chi, số dư tạm tính và lịch sử giao dịch.',
    status: 'planned',
    plannedFeatures: [
      'Xem tổng thu',
      'Xem tổng chi',
      'Xem số dư tạm tính',
      'Theo dõi lịch sử giao dịch',
    ],
    plannedData: [
      'Tổng thu',
      'Tổng chi',
      'Số dư tạm tính',
      'Lịch sử giao dịch',
    ],
  },
  {
    id: 'kho-hang',
    name: 'Kho hàng',
    shortDescription: 'Chuẩn bị khung quản lý hàng hóa, nhập kho, xuất kho và tồn kho.',
    status: 'planned',
    plannedFeatures: [
      'Xem danh sách hàng hóa',
      'Nhập kho',
      'Xuất kho',
      'Theo dõi tồn kho',
    ],
    plannedData: [
      'Thông tin hàng hóa',
      'Phiếu nhập kho',
      'Phiếu xuất kho',
      'Số lượng tồn kho',
    ],
  },
  {
    id: 'bao-cao',
    name: 'Báo cáo',
    shortDescription: 'Chuẩn bị khu vực tổng hợp báo cáo ngày, tuần, tháng và cảnh báo.',
    status: 'planned',
    plannedFeatures: [
      'Báo cáo ngày',
      'Báo cáo tuần',
      'Tổng kết tháng',
      'Chuẩn bị biểu đồ/cảnh báo',
    ],
    plannedData: [
      'Số liệu ngày',
      'Số liệu tuần',
      'Tổng kết tháng',
      'Dữ liệu cảnh báo',
    ],
  },
  {
    id: 'cai-dat-co-so',
    name: 'Cài đặt cơ sở',
    shortDescription: 'Chuẩn bị cấu hình thông tin cơ sở và các dữ liệu nền cho vận hành.',
    status: 'planned',
    plannedFeatures: [
      'Thông tin cơ sở',
      'Cấu hình gói học',
      'Cấu hình trạng thái học viên',
      'Cấu hình dữ liệu mẫu',
    ],
    plannedData: [
      'Thông tin cơ sở',
      'Danh mục gói học',
      'Danh mục trạng thái học viên',
      'Dữ liệu mẫu',
    ],
  },
  {
    id: 'dang-cap-nhat',
    name: 'Đang cập nhật',
    shortDescription: 'Nơi giữ các module sẽ bổ sung sau khi phạm vi sản phẩm rõ hơn.',
    status: 'placeholder',
    plannedFeatures: ['Nơi giữ các module sẽ bổ sung sau'],
    plannedData: ['Danh sách ý tưởng module', 'Ghi chú phạm vi tương lai'],
  },
]
