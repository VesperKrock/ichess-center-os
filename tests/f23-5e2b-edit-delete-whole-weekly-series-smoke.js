...
assert(actionFlow.includes("scheduleCalendarItemState?.mode === 'seriesDelete'"), 'Delete must retain the confirmed master/version instead of adopting a refreshed version.')
assert(actionFlow.includes('buildC57ArchiveCalendarItemCommand(masterItem)'), 'Delete must archive exactly the authoritative master through C5.7.')
