...
assert(
  ...
  'C6.5C runtime diff must be committed or stay in main.js/styles.css only',
)
...
for (const line of status.split(/\r?\n/).filter(Boolean)) {
  ...
  assert(!/c6-5(?![abcd])|c6-6(?![abcde])|c7|teacher-portal|super-admin/i.test(changedPath), `C6.5C must not create future scope files: ${changedPath}`)
}
