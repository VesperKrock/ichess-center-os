...
for (const line of status.split(/\r?\n/).filter(Boolean)) {
  ...
  assert(!/\.sql$/i.test(changedPath) || /supabase-c6-3(b-(readonly-inspect-centers-schema|manual-apply-centers-schema-hardening-template)|c-readonly-verify-centers-schema-hardening-applied)\.sql$/i.test(changedPath), `C6.2A must not add SQL: ${changedPath}`)
  assert(!/c6-3(?![abcde])|c6-4|c6-5|internal-centers|c7|teacher-portal|super-admin/i.test(changedPath), `C6.2A must not create future scope files: ${changedPath}`)
}
