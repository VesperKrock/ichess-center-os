...
assert.equal(process.env.ICHESS_F1_5_LOCAL_QA_ALLOW_MUTATION, 'YES')
...
const sql = readFileSync('tests/f1-5-schedule-centered-instructor-cleanup-local-db-qa.sql', 'utf8')
requireSuccess(run('docker', [
  'exec', '-i', containers[0][0], 'psql', '-X', '--no-psqlrc',
  '-U', 'postgres', '-d', 'postgres', '-v', 'ON_ERROR_STOP=1', '-q',
], { input: sql }), 'F1.5 transactional QA')
...
const residue = requireSuccess(run('docker', [
  ...
  input: `
    select
      (select count(*) from public.centers where id like 'f1_5_qa_%')
      + (select count(*) from public.center_members where center_id like 'f1_5_qa_%')
      + (select count(*) from public.center_cloud_entities where center_id like 'f1_5_qa_%');
  `,
}), 'F1.5 residue check').trim()
assert.equal(residue, '0')

console.log('F1_5_SCHEDULE_CENTERED_INSTRUCTOR_CLEANUP_LOCAL_DB_QA: PASS (residue=0)')
