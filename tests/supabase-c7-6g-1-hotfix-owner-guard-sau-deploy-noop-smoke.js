...
const docPath = path.join(root, 'docs', 'supabase-c7-6g-1-hotfix-owner-guard-sau-deploy-noop.md')
const functionPath = path.join(root, 'supabase', 'functions', 'provision-center-admin-account', 'index.ts')
const testPath = path.join(root, 'tests', 'supabase-c7-6g-1-hotfix-owner-guard-sau-deploy-noop-smoke.js')
...
assert(fs.existsSync(docPath), 'Docs C7.6G.1 must exist')
assert(fs.existsSync(functionPath), 'Edge Function source must exist')
assert(fs.existsSync(testPath), 'Smoke C7.6G.1 must exist')
...
assertNotIncludes(fn, 'owner_guard_failed', 'ambiguous owner_guard_failed in function source')
...
for (const changedPath of changedPaths) {
  ...
  assert(allowedPaths.has(changedPath), `Unexpected changed file in C7.6G.1 scope: ${changedPath}`)
  assert(!changedPath.startsWith('src/'), `No src runtime diff allowed in C7.6G.1: ${changedPath}`)
...
console.log('C7.6G.1 smoke: PASS')
