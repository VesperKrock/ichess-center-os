-- OV1.4: a narrow, audited reversal path for an erroneous tuition payment.
-- Generic Finance edits remain protected; this RPC only permits POSTED -> VOIDED.
...
begin
  ...
  where e.center_id = v_center_id
    ...
    and e.local_id = v_source_tuition_id
    and e.deleted_at is null
  for share;
  if not found
     or pg_catalog.btrim(coalesce(v_tuition_entity.payload->>'id', '')) <> v_source_tuition_id
...
comment on function public.c5_4_void_tuition_payment(
  text, uuid, text, text, bigint, text, uuid
) is 'OV1.4 exact-center audited POSTED-to-VOIDED reversal for a protected tuition payment.';
