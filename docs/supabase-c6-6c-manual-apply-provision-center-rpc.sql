...
-- Data impact: does not create centers by itself; does not create memberships by itself; creates/replaces helper/RPC functions and grants execute permission.
-- Destructive impact: no table drops, no data deletion, no data migration.
-- Backup: recommended schema backup/snapshot before applying.
-- Apply order:
-- 1) Review this SQL.
-- 2) Apply in Supabase SQL Editor only after user confirmation.
-- 3) Run post-apply verify SQL.
-- 4) Do not create a real center until runtime/test phase confirms RPC behavior.
...
is 'C6.6 helper: compact Vietnamese center slug, no accents, no hyphen.';

comment on function public.provision_center_for_owner(text)
is 'C6.6 guarded RPC: one visible input p_center_name, provisions empty production center metadata and owner membership when explicitly called after apply.';
