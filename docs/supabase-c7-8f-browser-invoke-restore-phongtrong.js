...
(async () => {
  ...
  const body = {
    ...
    idempotency_key: `c7-8f-restore-phongtrong-prod-${Date.now()}`,
    reason: 'owner_controlled_restore_center_admin_access',
  }

  console.log('Invoking controlled restore for Phong Trong only:', {
    center_id: body.center_id,
    target_email: body.target_email,
    idempotency_key: body.idempotency_key,
  })

  const response = await fetch('https://zahcfnpaprbnuqpegdmo.supabase.co/functions/v1/restore-center-admin-access', {
    method: 'POST',
    headers: {
  ...
  console.log('Controlled restore response:', {
